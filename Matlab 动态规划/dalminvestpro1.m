...
x(i,:)=linspace(umin(i),umin(i)+50,100);
%y1=sqrt(a*(x1-umin).^2+vmin);
y(i,:)=a(i)*(x(i,:)-umin(i)).^2+vmin(i);
end

XRb=XL(:,2:5)*ones(1,n1-1)'/(n1-1);

xk(1)=x0;lk(1)=l0;Sk(1)=x0-l0;
t(1)=0;
if XRb(1)<=d
        ms0=1;
    end
...
for k=1:12
    t(k+1)=k;
    if XRb(k)<=d
        ms=1;marketstate(k)=1;
    end
    if XRb(k)>d
        ms=2;marketstate(k)=2;
    end
...
q=XL(k,5);
P=e-sk(ms)
uk(:,k)=lw(ms,k)* X1(:,ms)*lk(k)+lw(ms,k)* X2(:,ms)+ X3(:,ms)+(xk(k)*sk(ms)+hw(ms,k)-(ud(ms0)-2*Ed)*nw(ms,k)/phi02(ms0))* X4(:,ms)
uk0(ii,k)=xk(k)-sum(uk(:,k));
pik(:,k)=uk(:,k)/xk(k);
pi0(ii,k)=uk0(ii,k)/xk(k);
xk(k+1)=xk(k)*sk(ms)+P*uk(:,k)+c;
lk(k+1)=lk(k)*q+b;
Sk(ii,k+1)=xk(k+1)-lk(k+1);
end
    end
uk,uk0,xk,lk,Sk,marketstate
 hold on;box on;
ylabel('Proportions invested in risk-free asset with differenct E[S_T]');xlabel('Mohth');
k=t(1:12)
plot(k,pi0(1,:),'k--.'),
plot(k,pi0(2,:),'g-','linewidth',3),
plot(k,pi0(3,:),'m:'),
plot(k,pi0(4,:),'r-.','linewidth',2),
plot(k,pi0(5,:),'b--','linewidth',2),
%plot(t,Sk(6,:),'r-.','linewidth',2),

legend('E[S_T]=20','E[S_T]=40','E[S_T]=80','E[S_T]=160','E[S_T]=320',4)
